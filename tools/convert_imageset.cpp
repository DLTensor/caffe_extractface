...
int main(int argc, char** argv) {
  ...
  */
  ...
  while (infile >> filename >> x1 >> y1 >> x2 >> y2){
	...
	// LOG(INFO) << "label size " << vec_label.size();
	// LOG(ERROR) << filename << " " << vec_label[0] << " " << vec_label[1] << " " << vec_label[2] << " " << vec_label[3];
  ...
  for (int line_id = 0; line_id < lines.size(); ++line_id) {
	// LOG(INFO) << "lines[line_id].second.size()=" << lines[line_id].second.size();
