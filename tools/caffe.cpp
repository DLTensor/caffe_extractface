...
int test() {
  ...
  for (int i = 0; i < FLAGS_iterations; ++i) {
	...
	// liu
	const shared_ptr<Blob<float> > feature_blob = caffe_net.blob_by_name("fc_8_det");
	...
	for (int n = 0; n < batch_size; ++n) {
		...
		//LOG(ERROR) << "dim_features " << dim_features \
				<< " " << feature_blob_data[0] << " " << feature_blob_data[1] \
				<< " " << feature_blob_data[2] << " " << feature_blob_data[3];
		int lines_id = image_layer->lines_id_;
		LOG(ERROR) << "lines_id_ " << lines_id << " lines_ " << image_layer->lines_[lines_id].first;
	}
	// end */
  }
